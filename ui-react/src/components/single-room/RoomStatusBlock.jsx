...
const RoomStatusBlock = ({ config, details, room }) => {
  ...
  const safeDetails = {
    ...
    appointmentExists: Boolean(
      currentDetails.appointmentExists &&
      Array.isArray(room.Appointments) &&
  ...
  return (
    <div className={room.Busy ? 'columns small-7 left-col busy' : 'columns small-7 left-col open'}>
      <div id="single-room__room-name">{room.Name}</div>
      <div id="single-room__room-status">{statusText}</div>

      <Details room={room} details={safeDetails} />
      <Time room={room} details={safeDetails} />
      <Organizer room={room} details={safeDetails} />
    </div>
  );
};
